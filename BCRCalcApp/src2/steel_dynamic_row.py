...
from steel_data_elem_num_nm import SteelElementsMData
from steel_data_defects import SteelDefectsData
from steel_data_units import SteelUnitsDB
from steel_data_cs import SteelConditionStateData
from steel_data_cs import retrieve_data_by_bid_item_num
import sqlite3
from tkinter import messagebox
from steel_validations import validate_total_quantity

class SteelDynamicRow(ttk.Frame):
    def __init__(self, master, container, controller, bridgeId, uuid, *args, **kwargs):
        ...
        self.allSteelElemsData = SteelElementsMData()
        self.elemNumDrpDnValues = self.allSteelElemsData.get_elementsNumList()
        self.allSteelUnitsDict = SteelUnitsDB().getSteelUnitsList()
        self.allSteelDefectsData = SteelDefectsData()
        self.defectsList = self.allSteelDefectsData.getOnlyDefectsList()

        self.SteelDefectsState1=self.allSteelDefectsData.getDefectListForState(1)
        self.SteelDefectsState2=self.allSteelDefectsData.getDefectListForState(2)
        self.SteelDefectsState3=self.allSteelDefectsData.getDefectListForState(3)
        self.SteelDefectsState4=self.allSteelDefectsData.getDefectListForState(4)
    ...
    def create_bridge_information_widgets(self):
        ...
        self.units_dropdown = ttk.Combobox(self.bridge_info_frame, textvariable=self.units_var, values=list(self.allSteelUnitsDict.keys()), state="disabled")
    ...
    def on_element_num_selected(self, event):
        ...
        if selected_option == 'Others':
            ...
            self.element_type_var=self.allSteelElemsData.get_element(selected_option).element_name
            self.element_type_entry['state'] = 'normal'
    ...
    def get_defect_bid_items_str(self, cs_index, selected_defect_name):
        if cs_index == 1:
            return self.SteelDefectsState1[selected_defect_name]
        elif cs_index == 2:
            return self.SteelDefectsState2[selected_defect_name]
        elif cs_index == 3:
            return self.SteelDefectsState3[selected_defect_name]
        elif cs_index == 4:
            return self.SteelDefectsState4[selected_defect_name]
        else:
            return "None"
    #########################################################################        
